					 ...
					 unsigned int reg, unsigned int exp_val)
{
	...
	time_out += jiffies;
	do {
		val = CX9000_OSS_STATE_GET(snd_soc_component_read(component, reg));
			    ...
			    struct snd_kcontrol *kcontrol, int event)
{
	...
	lp_mode = snd_soc_component_read(component, CX9000_OSS_CSR);
...
static void cx9000_wait_for_coeff_writing(struct snd_soc_component *component)
{
	...
	time_out += jiffies;
	do {
		done = snd_soc_component_read(component, CX9000_COEF_CTRL0);
			      ...
			      struct snd_ctl_elem_value *ucontrol)
{
	...
	ucontrol->value.integer.value[0] =
		!((snd_soc_component_read(component, CX9000_HPF_LEFT_CONTROL) >> 6) & 0x01);
	ucontrol->value.integer.value[1] =
		!((snd_soc_component_read(component, CX9000_HPF_RIGHT_CONTROL) >> 6) & 0x01);
			      ...
			      struct snd_ctl_elem_value *ucontrol)
{
	...
	ucontrol->value.integer.value[0] =
		(snd_soc_component_read(component, CX9000_HPF_LEFT_CONTROL) >> 5) & 0x01;
	ucontrol->value.integer.value[1] =
		(snd_soc_component_read(component, CX9000_HPF_RIGHT_CONTROL) >> 5) & 0x01;
			      ...
			      struct snd_ctl_elem_value *ucontrol)
{
	...
	ucontrol->value.integer.value[0] =
		snd_soc_component_read(component, CX9000_HPF_LEFT_CONTROL) &
			CX9000_HPF_CUTOFF_MASK;
	ucontrol->value.integer.value[1] =
		snd_soc_component_read(component, CX9000_HPF_RIGHT_CONTROL) &
			CX9000_HPF_CUTOFF_MASK;
...
static int cx9000_codec_probe(struct snd_soc_component *component)
{
	...
	val = snd_soc_component_read(component, CX9000_OSS_CSR);
...
#define RETURN_VALUE	
#endif
static RET_TYPE cx9000_i2c_remove(struct i2c_client *client)
{
