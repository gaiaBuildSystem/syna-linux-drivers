				...
				struct snd_soc_dai *dai)
{
	...
	aio_i2s_set_clock(dmic->aio_handle, AIO_ID_PDM_RX, dmic_clk_table[config_index].clk_sel,
				  AIO_CLK_D3_SWITCH_NOR, AIO_CLK_SEL_D8, AIO_APLL_0, 1);
...
static int berlin_dmic_pdm_suspend(struct device *dev)
{
	...
	aio_i2s_mclk_ctrl(dmic->aio_handle, AIO_ID_PDM_RX, false);

	return ret;
}

static int berlin_dmic_pdm_resume(struct device *dev)
{
	struct dmic_data *dmic;
	int i, ret = 0;
	...
	aio_i2s_mclk_ctrl(dmic->aio_handle, AIO_ID_PDM_RX, true);
	return 0;
}
#endif

static SIMPLE_DEV_PM_OPS(berlin_dmic_pdm_pmops, berlin_dmic_pdm_suspend,
			 berlin_dmic_pdm_resume);
...
static struct snd_soc_dai_driver dmic_pdm_dai = {
	...
	.capture = {
		...
		.rates = DMIC_RECORD_RATES,
		.formats = DMIC_RECORD_FORMATS,
	},
...
static RET_TYPE dmic_pdm_remove(struct platform_device *pdev)
{
	struct device *dev = &pdev->dev;
	struct dmic_data *dmic;
