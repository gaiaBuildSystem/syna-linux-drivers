...
static int i2s_mic2_component_hw_free(struct snd_soc_component *component,
							struct snd_pcm_substream *substream)
{
	...
	mic2 = snd_soc_dai_get_drvdata(cpu_dai);
	if (mic2 == NULL)
		return 0;

	if (mic2->irq_requested && mic2->irq >= 0) {
		berlin_pcm_free_dma_irq(substream, 1, &mic2->irq);
		mic2->irq_requested = false;
	}
...
static const struct snd_soc_component_driver i2s_mic2_component = {
	.name = "i2s-mic2",
	.hw_free = i2s_mic2_component_hw_free,
};
...
static RET_TYPE i2s_mic2_remove(struct platform_device *pdev)
{
	struct device *dev = &pdev->dev;
	struct mic2_priv *mic2;
...
static const struct of_device_id i2s_mic2_dt_ids[] = {
	...
	{ .compatible = "syna,platypus-i2s-mic2",  },
	{ .compatible = "syna,myna-mic2",  },
	{ .compatible = "syna,myna2-mic2",  },
	{}
};
