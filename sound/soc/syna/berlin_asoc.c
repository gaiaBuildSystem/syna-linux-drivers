				    ...
				    struct snd_pcm_hw_params *params)
{
	...
	if (rtd->num_codecs == 2) {
		...
		if (strstr(codec->name, "cx2072x")) {
			cret = snd_soc_dai_set_sysclk(codec, 1, 12288000, 0);
			if (cret)
				dev_err(rtd->dev, "error %d set codec %s\n",
					cret, codec->name);
			cret = snd_soc_dai_set_bclk_ratio(codec, 64);
		...
		if (strstr(codec->name, "tlv320aic3x")) {
			...
			cret = snd_soc_dai_set_sysclk(codec, 0, rate, 0);
			if (cret)
				dev_err(rtd->dev, "error %d set codec %s\n",
					cret, codec->name);
		}
...
static int berlin_asoc_probe(struct platform_device *pdev)
{
	...
	for_each_child_of_node(np, iter) {
		...
		/* Get dai fmt */
		plink->dai_fmt = snd_soc_daifmt_parse_format(iter, NULL);
		plink->dai_fmt |= snd_soc_daifmt_parse_clock_provider_as_flag(iter, NULL);
		snd_printk("dai %d: dai_fmt 0x%X cpu dai %s codec dai %s\n",
				priv->link_num,
				plink->dai_fmt,
				(plink->cpus->name != NULL) ?
						plink->cpus->name : plink->cpus->of_node->name,
