...
static irqreturn_t sunplus_handle_interrupt(struct phy_device *phydev)
{
	...
	phy_write(phydev, SUNPLUS_INT_STS, irq_status);
	...
	if (irq_status & LNK_STS_CHG) {
		...
		if (sts & FINAL_LINK) {
			...
			if (time_after(jiffies, last_jiffies + msecs_to_jiffies(50))) {
...
static int sunplus_config_intr(struct phy_device *phydev)
{
	...
	if (phydev->interrupts == PHY_INTERRUPT_ENABLED) {
		err = sunplus_ack_interrupt(phydev);
		if (err)
			return err;
		val = LNK_STS_CHG | MGC_PKT_DET;
		err = phy_write(phydev, SUNPLUS_INT_MASK, val);
	} else {
		val = 0;
		err = phy_write(phydev, SUNPLUS_INT_MASK, val);
		if (err)
...
static struct phy_driver sunplus_drvs[] = {
	{
		...
		.handle_interrupt	= sunplus_handle_interrupt,
...
static void sunplus_ephy_shutdown(struct platform_device *pdev)
{
	struct sunplus_ephy *priv = platform_get_drvdata(pdev);
...
static struct platform_driver sunplus_ephy_driver = {
	...
	.shutdown = sunplus_ephy_shutdown,
};
...
MODULE_SOFTDEP("post: dwmac-generic");
