...
struct ebg_file_header {
    ...
    uint32_t public_data_length;
    uint32_t vsi_data_offset;
};
...
struct ebg_file_vsi_data_header {
...
#define EBG_FILE_VSI_DATA_HEADER_SIZE ( 4 * 4 )

enum {
    EBG_FILE_EXECUTION_MODE_NORMAL = 0,
    EBG_FILE_EXECUTION_MODE_PROFILE = 1
};

enum {
    EBG_FILE_SECURITY_NO_SECURITY = 0,
    EBG_FILE_SECURITY_SECURE = 1
};
...
bool ebg_file_parse_header(uint8_t *ebg, size_t len, struct ebg_file_header* header);

/// parses all the clear information in the EBG file
/// returns true in case of success, false otherwise
bool ebg_file_parse_public_data(uint8_t *ebg, size_t len, struct ebg_file_public_data* clear_data);
...
/// parse the VSI reserved data header in the model file
/// returns true in case of success, false otherwise
bool ebg_file_parse_vsi_data_header(uint8_t* ebg_file_head, size_t ebg_file_head_size,
                                    struct ebg_file_public_data *pd,
                                    struct ebg_file_vsi_data_header* vsi_data);

/// parse the layers in the model file
/// returns true in case of success, false otherwise
bool ebg_file_parse_layers(uint8_t* ebg_file_head, size_t ebg_file_head_size,
                           struct ebg_file_public_data *pd,
                           struct ebg_file_vsi_data_header *vsi_data,
                           struct ebg_file_layer *layers,
                           size_t layer_count);

/// parse the operations in the model file
/// returns true in case of success, false otherwise
bool ebg_file_parse_operations(uint8_t* ebg_file_head, size_t ebg_file_head_size,
                               struct ebg_file_public_data *pd,
                               struct ebg_file_vsi_data_header *vsi_data,
