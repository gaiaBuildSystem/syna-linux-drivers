...
bool synap_profile_parse_layers(void* model_buffer, size_t model_size,
                                ...
                                struct synap_network_resources_desc *desc)
{
    ...
    if (!ebg_file_parse_vsi_data_header(model_buffer, model_size, pd, &vsi_data)) {
        return false;
    }

    layer_data_size = sizeof(struct ebg_file_layer) * vsi_data.layer_count;

    layer_data = vzalloc(layer_data_size);
    if (!layer_data) {
        KLOGE("failed to alloc layers buf");
        return false;
    }

    if (!ebg_file_parse_layers(model_buffer, model_size, pd, &vsi_data,
                               layer_data, vsi_data.layer_count)) {
        KLOGE("failed to get layer data");
        vfree(layer_data);
        return false;
    }

    layer_profile = vzalloc(sizeof(struct synap_profile_layer) * vsi_data.layer_count);
    if (!layer_profile) {
        KLOGE("failed to alloc layers buf");
        vfree(layer_data);
        return false;
...
bool synap_profile_get_layers_data(struct synap_network *network)
{
    ...
    if (compatible_dma_buf_vmap(dmabuf, &map) < 0) {
    ...
    for (i = 0; i < network->operation_count; i++) {
        ...
        else if (p->layer_idx != 0xffffffff) {
            // Only give error message if layer_idx is not -1 (INIT)
    ...
    compatible_dma_buf_vunmap(dmabuf, &map);
