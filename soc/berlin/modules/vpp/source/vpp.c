...
{
	//Force enable only, don't allow force disable.
	if (enable)
		recovery = enable;
}
EXPORT_SYMBOL(vpp_force_enable_recovery);

int VPP_Is_Recovery_Mode(void)
{
	return recovery;
}
EXPORT_SYMBOL(VPP_Is_Recovery_Mode);

int is_ampless_boot(void)
{
	return enable_ampless_boot;
}
EXPORT_SYMBOL(is_ampless_boot);
...
static void vpp_read_config(struct platform_device *pdev)
{
	struct device_node *np;

	np = pdev->dev.of_node;
	of_property_read_u32(np, "enable_ampless_boot",
			&enable_ampless_boot);

	//Force recovery flow for ampless boot
	vpp_force_enable_recovery(enable_ampless_boot);

	pr_info("%s:%d: enable_ampless_boot - %x, is_recovery - %x\n",
		__func__, __LINE__, enable_ampless_boot, recovery);
}
...
static int syna_vpp_platform_probe(struct platform_device *pdev)
{
	...
	//read enable_ampless_boot from dts file
	vpp_read_config(pdev);
	...
	pr_info("vpp : probe successful, mode: %s\n", recovery ? "RECOVERY" : "NORMAL");
	vpp_initialized = 1;
	return 0;
}

static RET_TYPE syna_vpp_platform_remove(struct platform_device *pdev)
{
	vpp_initialized = 0;
	if (recovery) {
...
MODULE_DEVICE_TABLE(of, syna_vpp_of_match);
...
module_platform_driver(syna_vpp_platform_driver);
MODULE_LICENSE("GPL");

EXPORT_SYMBOL(VPP_Clock_Set_Rate_Ext);
EXPORT_SYMBOL(VPP_Clock_Set_Rate);
