...
{
	return (void *)gIspDhubCtx;
}
EXPORT_SYMBOL(isp_dhub_get_mode_ctx);

int isp_dhub_write_reg(unsigned int offset, unsigned int val)
{
	ISP_DHUB_CTX *hDhubCtx = (ISP_DHUB_CTX *)gIspDhubCtx->mod_prv;
	void __iomem *final_addr = (void __iomem *)hDhubCtx->mod_base + offset;

	writel(val, final_addr);
	return 0;
}
EXPORT_SYMBOL(isp_dhub_write_reg);

int isp_dhub_read_reg(unsigned int offset)
{
	ISP_DHUB_CTX *hDhubCtx = (ISP_DHUB_CTX *)gIspDhubCtx->mod_prv;
	unsigned int val = 0;
	void __iomem *final_addr = (void __iomem *)hDhubCtx->mod_base + offset;

	val = readl(final_addr);
	return val;
}
EXPORT_SYMBOL(isp_dhub_read_reg);
...
static void isp_drv_dhub_update_intr_period(ISP_DHUB_INTR_HANDLER_INFO *p_int_handler)
{
	...
	struct timespec64 nows;

	ktime_get_ts64(&nows);
	time = timespec64_to_ns(&nows);
#else
			...
			unsigned int intr_num)
{
	...
	if ((hDhubCtx->intrMask & intr_mask)) {
		...
		ispdhub_debug("%s:%d Removed fom ISP_DHUB-/%d : %d / %x\n",
			__func__, __LINE__,
			...
			void *pArgs, ISP_DHUB_INTR_HANDLER intrHandler)
{
	...
	if (!(hDhubCtx->intrMask & intr_mask)) {
		...
		ispdhub_debug("%s:%d Added to ISP_DHUB-/%d : %d / %x\n",
			__func__, __LINE__,
			hDhubCtx->intrHandlerCount, intr_num, intr_mask);
...
static int isp_drv_dhub_pop_msg(ISP_DHUB_CTX *hDhubCtx, CC_MSG_t *p_msg)
{
	int ret = 0;
	unsigned long flags;
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0)
static const struct proc_ops isp_dhub_driver_config_fops = {
	.proc_open = isp_dhub_config_proc_open,
	.proc_read = seq_read,
	.proc_lseek = seq_lseek,
	.proc_release = single_release,
	.proc_write = isp_dhub_config_proc_write,
};

static const struct proc_ops isp_dhub_driver_status_fops = {
	.proc_open = isp_dhub_status_proc_open,
	.proc_read = seq_read,
	.proc_lseek = seq_lseek,
	.proc_release = single_release,
};

static const struct proc_ops isp_dhub_driver_detail_fops = {
	.proc_open = isp_dhub_detail_proc_open,
	.proc_read = seq_read,
	.proc_lseek = seq_lseek,
	.proc_release = single_release,
};
...
long isp_dhub_mod_func(isp_module_ctx *mod_ctx,
								unsigned int cmd, void *arg)
{
	...
	{
		unsigned int *aff_param = (unsigned int *)arg;
		const struct cpumask *m;

		ispdhub_debug("%s:%d: aff_param-%d,%d\n",
			__func__, __LINE__, aff_param[0], aff_param[1]);

		ispdhub_debug("%s:%d: set aff_param to irq - %d\n",
	...
	case ISP_DHUB_IOCTL_ENABLE_INT:
	{
		...
		ispdhub_debug("%s:%d: Enable int - %d / 0x%x\n",
	...
	case ISP_DHUB_IOCTL_DISABLE_INT:
	{
		...
		ispdhub_debug("%s:%d: Disable int - %d / 0x%x\n",
			__func__, __LINE__, irq_num, 1 << irq_num);
		...
		//Trigger the out-thread to come out
		up(&hDhubCtx->dhub_sem);
...
EXPORT_SYMBOL(isp_dhub_mod_func);
			...
			unsigned int cmd, unsigned long arg)
{
	...
	case ISP_DHUB_IOCTL_SET_AFFINITY:
	{
		...
		isp_dhub_mod_func(mod_ctx, sub_cmd, aff_param);
		break;
	}

	case ISP_DHUB_IOCTL_DHUB_INIT:
	{
		isp_dhub_mod_func(mod_ctx, sub_cmd, NULL);
		break;
	}
	...
	case ISP_DHUB_IOCTL_ENABLE_INT:
	{
		...
		isp_dhub_mod_func(mod_ctx, sub_cmd, &irq_num);
		break;
	}
	...
	case ISP_DHUB_IOCTL_WAIT_FOR_INT:
	{
		...
		ret = isp_dhub_mod_func(mod_ctx, sub_cmd, &msg);
...
EXPORT_SYMBOL(isp_dhub_mod_close);
...
int isp_dhub_mod_open(isp_device *isp_dev, isp_module_ctx *mod_ctx)
{
	...
	if (((int)vec_num) > 0 && (hDhubCtx->dhub_id < ISP_DHUB_ID_MAX)) {
		...
		if (unlikely(err < 0))
			ispdhub_error("vec_num:%5d, err:%8x\n", vec_num, err);
		else
...
EXPORT_SYMBOL(isp_dhub_mod_open);
