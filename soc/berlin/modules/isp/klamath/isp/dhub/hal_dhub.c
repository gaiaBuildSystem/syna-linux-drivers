...
#define       CSIPIPE_DHUB              0xF7440000

#define bTST(x, b)      (((x) >> (b)) & 1)

/* Stub implementations for missing BCM and register functions */
static u32 getBcmBaseAddr(void) { return VIP_BCMQ_OFFSET; }

#define IO32WR(d, a) HAL_ISP_DHUB_REG_WRITE32(a, d)
#define IO32RD(d, a) HAL_ISP_DHUB_REG_READ32(a, d)

#define IO32CFG(cfgQ, i, a, d, base)  do { \
	if (cfgQ) {\
		(cfgQ)[i][0] = (d);\
		(cfgQ)[i][1] = (UNSG32)(a - base + CSIPIPE_DHUB + CSI_DHUB_BASE);\
	} else\
		HAL_ISP_DHUB_REG_WRITE32(a, d); \
	(i)++;\
} while (0)
 ...
 ***********************************************************************/
void semaphore_hdl(UNSG64 ra, void *hdl)
{
...
UNSG32 semaphore_cfg(void *hdl, SIGN32 id, SIGN32 depth, T64b cfgQ[])
{
	...
	UNSG32 i = 0;
	UNSG64 a = sem->ra + RA_SemaHub_ARR + id*sizeof(SIE_Semaphore);

	cfg.u32 = 0; cfg.uCFG_DEPTH = sem->depth[id] = depth;
	IO32CFG(cfgQ, i, a + RA_Semaphore_CFG, cfg.u32, sem->ra);
...
/*********************************************************************************
 * Function: isp_semaphore_chk_full
...
UNSG32	isp_semaphore_chk_full(void *hdl, SIGN32 id)
{
	...
	return (id < 0) ? d : bTST(d, id);
	/**	ENDOFFUNCTION: isp_semaphore_chk_full **/
}
EXPORT_SYMBOL(isp_semaphore_chk_full);
...
int dhub_wait_complete(int ch, struct HDL_semaphore *pSemHandle, int timeout)
{
	...
	do {
		dHubStatus = isp_semaphore_chk_full(pSemHandle, -1);
		if (dHubStatus & (1<<ch))
			break;
	...
	if (isp_semaphore_chk_full(pSemHandle, ch)) {
 ...
 ***********************************************************/
void hbo_hdl(UNSG64	mem, UNSG64 ra, void *hdl)
{
		...
		SIGN32 depth, SIGN32 enable, T64b cfgQ[])
{
	...
	IO32CFG(cfgQ, i, a + RA_FiFo_START, 0, hbo->ra);

	cfg.u32 = 0; cfg.uCFG_BASE = hbo->base[id] = base;
	IO32CFG(cfgQ, i, a + RA_FiFo_CFG, cfg.u32, hbo->ra);

	i += semaphore_cfg(fifoCtl, id, depth, cfgQ ? (cfgQ + i) : NULL);
	IO32CFG(cfgQ, i, a + RA_FiFo_START, enable, hbo->ra);
...
{
	struct HDL_hbo *hbo = (struct HDL_hbo *)hdl;
	UNSG64 a = hbo->ra + RA_HBO_ARR + id*sizeof(SIE_FiFo);
		...
		T64b data[], T64b cfgQ[], UNSG32 *ptr)
{
	...
	for (i = 0; i < n; i++) {
		IO32CFG(cfgQ, j, base + p*8, data[i][0], hbo->mem);
		IO32CFG(cfgQ, j, base + p*8 + 4, data[i][1], hbo->mem);
		ModInc(p, 1, depth);
	}
	if (!cfgQ)
		...
		IO32CFG(cfgQ, j, hbo->fifoCtl.ra + RA_SemaHub_PUSH, push.u32, hbo->fifoCtl.ra);
	}
	if (ptr)
		*ptr = p;
	return j;
 ...
 * void *hdl - Handle to HDL_dhub2d
 *******************************************************************/
void dhub_hdl(UNSG64 mem, UNSG64 ra, void *hdl)
{
...
EXPORT_SYMBOL(isp_dhub_semaphore);
		...
		T64b        cfgQ[])
{
	...
	pr_debug("hal_dhub::  value of Channel Addr    is %0lx\n", a);

	awQos.u32 = 0; awQos.uAWQOS_LO = awQosLO; awQos.uAWQOS_HI = awQosHI;
	pr_debug("hal_dhub::  addr of ChannelCFG is %0lx data is %0x\n",
			a + RA_dHubChannel_AWQOS, awQos.u32);
	IO32CFG(cfgQ, i, a + RA_dHubChannel_AWQOS, awQos.u32, dhub->ra);

	arQos.u32 = 0; arQos.uARQOS_LO = arQosLO; arQos.uARQOS_HI = arQosHI;
	pr_debug("hal_dhub::  addr of ChannelCFG is %0lx data is %0x\n",
			a + RA_dHubChannel_ARQOS, arQos.u32);
	IO32CFG(cfgQ, i, a + RA_dHubChannel_ARQOS, arQos.u32, dhub->ra);
		...
		T64b		cfgQ[])
{
	...
	UNSG32 i = 0, busyStatus, cmdID = dhub_id2hbo_cmdQ(id), dataID = dhub_id2hbo_data(id);
	UNSG64 a;
	...
	a = dhub->ra + RA_dHubReg_ARR + id*sizeof(SIE_dHubChannel);
	pr_debug("hal_dhub::  value of Channel Addr    is %0lx\n", a);
	IO32CFG(cfgQ, i, a + RA_dHubChannel_START, 0, dhub->ra);
	...
	IO32CFG(cfgQ, i, a + RA_dHubChannel_CFG, cfg.u32, dhub->ra);
	...
	pr_debug("hal_dhub::  addr of ChannelEN is %0lx data is %0x\n",
			a + RA_dHubChannel_START, enable);
	IO32CFG(cfgQ, i, a + RA_dHubChannel_START, enable, dhub->ra);
...
{
	struct HDL_dhub *dhub = (struct HDL_dhub *)hdl;
	UNSG64 a = dhub->ra + RA_dHubReg_ARR + id*sizeof(SIE_dHubChannel);
 ...
 * void *hdl - Handle to HDL_dhub2d
 ******************************************************************/
void dhub2d_hdl(UNSG64 mem, UNSG64 ra, void *hdl)
{
		...
		T64b	cfgQ[])
{
	...
	IO32CFG(cfgQ, j, a + RA_dHubCmd2D_START, 0, dhub2d->ra);

	cmd.uMEM_addr = addr;
	cmd.uDESC_stride = stride; cmd.uDESC_numLine = height;
	cmd.uDESC_hdrLoop = semLoop; cmd.uDESC_interrupt = interrupt;
	IO32CFG(cfgQ, j, a + RA_dHubCmd2D_MEM, cmd.u32dHubCmd2D_MEM, dhub2d->ra);
	IO32CFG(cfgQ, j, a + RA_dHubCmd2D_DESC, cmd.u32dHubCmd2D_DESC, dhub2d->ra);
	...
	for (i = 0; i < semLoop; i++) {
		...
		IO32CFG(cfgQ, j, a + RA_dHubCmd2D_HDR
				...
				hdr.u32dHubCmdHDR_DESC, dhub2d->ra);
	}
	IO32CFG(cfgQ, j, a + RA_dHubCmd2D_START, enable, dhub2d->ra);
		...
		SIGN32 enable, T64b cfgQ[])
{
	...
	IO32CFG(cfgQ, i, a + RA_dHubCmd2D_START, enable, dhub2d->ra);
	return i;
}
...
{
	struct HDL_dhub2d *dhub2d = (struct HDL_dhub2d *)hdl;
	UNSG64 a = dhub2d->ra + RA_dHubReg2D_ARR + id*sizeof(SIE_dHubCmd2D);

	IO32WR(1, a + RA_dHubCmd2D_CLEAR);
}

/******************************************************************
 * Function: isp_dhub2nd_channel_cfg
 ...
 * Return: UNSG32 Number of (adr,pair) added to cfgQ
 *******************************************************************/
UNSG32 isp_dhub2nd_channel_cfg(
		void	*hdl,		/*! Handle to HDL_dhub2d !*/
		SIGN32	id,			/*! Channel ID in $dHubReg2D !*/
		...
		T64b	cfgQ[])
{
	...
	IO32CFG(cfgQ, j, a + RA_dHubCmd2ND_START, 0, dhub2d->ra);
	...
	IO32CFG(cfgQ, j, a, stdHubChannelRob_Map.u32, dhub2d->ra);

	a = dhub2d->ra + RA_dHubReg2D_ARR_2ND + id*sizeof(SIE_dHubCmd2ND);

	cmd.uMEM_addr = addr;
	pr_debug("addr 0x%lx: 0x%x\n", a + RA_dHubCmd2ND_MEM, addr);
	IO32CFG(cfgQ, j, a + RA_dHubCmd2ND_MEM, cmd.u32dHubCmd2ND_MEM, dhub2d->ra);
	...
	IO32CFG(cfgQ, j, a + RA_dHubCmd2ND_DESC, cmd.u32dHubCmd2ND_DESC, dhub2d->ra);

	memset(&cmd, 0, sizeof(SIE_dHubCmd2ND));
	cmd.uDESC_1D_ST_step = step1;
	IO32CFG(cfgQ, j, a + RA_dHubCmd2ND_DESC_1D_ST, cmd.u32dHubCmd2ND_DESC_1D_ST, dhub2d->ra);

	memset(&cmd, 0, sizeof(SIE_dHubCmd2ND));
	cmd.uDESC_1D_SZ_size = size1;
	IO32CFG(cfgQ, j, a + RA_dHubCmd2ND_DESC_1D_SZ, cmd.u32dHubCmd2ND_DESC_1D_SZ, dhub2d->ra);

	memset(&cmd, 0, sizeof(SIE_dHubCmd2ND));
	cmd.uDESC_2D_ST_step = step2;
	IO32CFG(cfgQ, j, a + RA_dHubCmd2ND_DESC_2D_ST, cmd.u32dHubCmd2ND_DESC_2D_ST, dhub2d->ra);

	memset(&cmd, 0, sizeof(SIE_dHubCmd2ND));
	cmd.uDESC_2D_SZ_size = size2;
	IO32CFG(cfgQ, j, a + RA_dHubCmd2ND_DESC_2D_SZ, cmd.u32dHubCmd2ND_DESC_2D_SZ, dhub2d->ra);

	IO32CFG(cfgQ, j, a + RA_dHubCmd2ND_START, enable, dhub2d->ra);
	...
	return j;
	/**	ENDOFFUNCTION: isp_dhub2nd_channel_cfg **/
}
EXPORT_SYMBOL(isp_dhub2nd_channel_cfg);
		...
		SIGN32 enable, T64b cfgQ[])
{
	...
	IO32CFG(cfgQ, i, a + RA_dHubCmd2ND_START, enable, dhub2d->ra);
	return i;
...
{
	struct HDL_dhub2d *dhub2d = (struct HDL_dhub2d *)hdl;
	UNSG64 a = dhub2d->ra + RA_dHubReg2D_ARR_2ND + id*sizeof(SIE_dHubCmd2ND);
 ...
 *	Description: Read dHub3D 'BUSY' status for all channel FIFOs.
 *  void *hdl - Handle to HDL_dhub2d
 *	Return: UNSG32 'BUSY' status bits of all channels
 ****************************************************************/
...
void BCM_SchedSetMux(struct camera_isp_dev *isp_dev, UNSG32 QID, UNSG32 TrigEvent)
{
	...
	if (QID <= BCM_SCHED_Q11) {
		...
		HAL_ISP_CORE_REG_WRITE32(isp_dev, bcmBaseAddr + addr, TrigEvent);
...
int BCM_SCHED_AutoPushCmd(struct camera_isp_dev *isp_dev, UNSG32 QID, UNSG8 uchEnable)
{
	...
	uiBcmQ_AutoPushSts = HAL_ISP_CORE_REG_READ32(isp_dev, bcmBaseAddr + RA_AVIO_BCM_AUTOPUSH);
	...
	//Write to Register
	HAL_ISP_CORE_REG_WRITE32(isp_dev, bcmBaseAddr + RA_AVIO_BCM_AUTOPUSH, uiBcmQ_AutoPushSts);
		...
		SIGN32 iMode, T64b cfgQ[])
{
	...
	csi_dhub_get_channel_info(hdl, id, &cfg);
...
int BCM_SCHED_PushCmd(struct camera_isp_dev *isp_dev, UNSG32 QID, UNSG32 *pCmd, UINT64 *cfgQ)
{
	...
	if (!cfgQ) {
		value = HAL_ISP_CORE_REG_READ32(isp_dev, bcmBaseAddr + RA_AVIO_BCM_FULL_STS);
		if (value & (1 << QID)) {
			pr_err("BCM Q Full!!\n");
	...
	if (pCmd) {
		...
		addr = QID * 4 * 2;
		if (cfgQ) {
			val = START_AVIO_VIP_BCMQ + addr;
			cfgQ[j] = val << 32 | pCmd[0];
			j++;
			val = START_AVIO_VIP_BCMQ + addr + 4;
			cfgQ[j] = val << 32 | pCmd[1];
			j++;
		} else {
			mutex_lock(&gBCM_SCHED_lock);
			HAL_ISP_CORE_REG_WRITE32(isp_dev, bcmBaseAddr + addr, pCmd[0]);
			HAL_ISP_CORE_REG_WRITE32(isp_dev, bcmBaseAddr + addr + 4, pCmd[1]);
			mutex_unlock(&gBCM_SCHED_lock);
			j = 2;
	pr_debug("cmd 0x%x 0x%x 0x%x\n", HAL_ISP_CORE_REG_READ32(isp_dev, bcmBaseAddr + 0),
			HAL_ISP_CORE_REG_READ32(isp_dev, bcmBaseAddr + 4),
			HAL_ISP_CORE_REG_READ32(isp_dev, bcmBaseAddr + 0x100));
		}
	}
...
void BCM_SCHED_GetEmptySts(struct camera_isp_dev *isp_dev, UNSG32 QID, UNSG32 *EmptySts)
{
	...
	bcmQ_sts = HAL_ISP_CORE_REG_READ32(isp_dev, bcmBaseAddr + RA_AVIO_BCM_EMP_STS);
	if (bcmQ_sts & (1 << QID))
		*EmptySts = 1;
	else
		*EmptySts = 0;
...
void BCM_SCHED_GetFullSts(struct camera_isp_dev *isp_dev, UNSG32 QID, UNSG32 *FullSts)
{
	...
	bcmQ_sts = HAL_ISP_CORE_REG_READ32(isp_dev, bcmBaseAddr + RA_AVIO_BCM_FULL_STS);
	if (bcmQ_sts & (1 << QID))
		*FullSts = 1;
	else
		*FullSts = 0;
...
void BCM_SCHED_QueueFlush(struct camera_isp_dev *isp_dev, UNSG32 QID)
{
    ...
    UNSG32 addr = RA_AVIO_BCM_FLUSH;
    UNSG32 value = 0;
    if (QID <= BCM_SCHED_Q18) {
        value = 1 << QID;
    }
    //TODO Handle flush for Q31
    HAL_ISP_CORE_REG_WRITE32(isp_dev, bcmBaseAddr + addr, value);
    return;
}
...
void BCM_SCHED_Flush(struct camera_isp_dev *isp_dev, UNSG32 mask)
{
	...
	HAL_ISP_CORE_REG_WRITE32(isp_dev, bcmBaseAddr + RA_AVIO_BCM_FLUSH, mask);
}
...
void bcm_buffer_write(void *pBcmBuf, UNSG32 regAddr, UNSG32 regVal)
{
	...
	/* Removed getBcmDhubType() check and dummy register handling */
...
int bcm_buffer_dummy_write(void *pBcmBuf, UNSG32 len)
{
	...
	if (len && len%4) {
		UNSG32 regAddr = 0x0; /* Dummy register address */
		UNSG32 regVal = 0x0;
