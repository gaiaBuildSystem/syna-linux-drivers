...
typedef struct isp_power_params_t {
	int enable;
	int dev_id;
}ISP_POWER_PARAMS;
			...
			unsigned int cmd, unsigned long arg)
{
	...
	case ISP_SET_POWER:
	{
		ISP_POWER_PARAMS power_par;

		if (!copy_from_user(&power_par, pInArg, sizeof(ISP_POWER_PARAMS))) {
			if (!IS_ERR_OR_NULL(isp_dev->enable_gpios)) {
				if (power_par.dev_id < isp_dev->enable_gpios->ndescs)
					gpiod_set_value_cansleep(
						isp_dev->enable_gpios->desc[power_par.dev_id],
										power_par.enable);
				else
					return -EINVAL;
			}
...
static int isp_probe(struct platform_device *pdev)
{
	...
	/* clock gate isp and csi block */
	writel(ISP_CLK_DISABLE, isp_dev->reg_base + ISP_GLOBAL_BASE +
				ISP_MISC_CLK_RESET_REG_BASE + ISP_CLK_CTRL_REG);
	writel(ISP_CLK_DISABLE, isp_dev->reg_base + ISP_GLOBAL_BASE +
				ISP_MISC_CLK_RESET_REG_BASE + CSI_CLK_CTRL_REG);
...
static int isp_suspend(struct device *dev)
{
	...
	// Disable all clocks & assert reset
	for (i = 0; i < ARRAY_SIZE(isp_clock_list); i++)
		clk_disable_unprepare(isp_clks[i]);

	reset_control_assert(isp_dev->rst);
...
static int isp_resume(struct device *dev)
{
	...
	// Deassert reset & enable all clocks
