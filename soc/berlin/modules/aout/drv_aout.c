...
static int user_aio_enabletxport(struct aout_priv *aout, u32 id, u32 enable)
{
	...
	ret = aio_enabletxport(aout->aio_hd, id, enable);

	if ((AIO_ID_SEC_TX == id) && (0 == ret)) {
		if (enable) {
...
static int user_aio_i2s_clk_enable(struct aout_priv *aout,
	u32 clk_idx, u32 enable)
{
	aio_set_i2s_clk_enable(aout->aio_hd, clk_idx, (bool)enable);
	return 0;
}

static int user_aio_pcm_mono_enable(struct aout_priv *aout,
	u32 path, u32 enable)
{
	aio_set_pcm_mono(aout->aio_hd, path, enable);
	return 0;
}
...
static struct function_table f_table[] = {
	...
	FUNC_ITEM(user_aio_i2s_clk_enable, 2),
	FUNC_ITEM(user_aio_pcm_mono_enable, 2),
...
static const struct proc_ops amp_driver_aout_fops = {
	.proc_open = aout_status_proc_open,
	.proc_read = seq_read,
...
static void drv_aout_release_shm(struct aout_priv *aout, int path_id)
{
	...
	dma_buf_vunmap(aout->ap[path_id].shm.buf, &map);
...
static int aout_probe(struct platform_device *pdev)
{
	...
	aout->dev = dev;
	aout->dev_name = AOUT_DEVICE_NAME;
