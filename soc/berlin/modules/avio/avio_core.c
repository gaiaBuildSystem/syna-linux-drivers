 ...
 */
 ...
 int __weak AVIO_MEM_IsReady(void);

static bool quiescent;
module_param(quiescent, bool, 0400);
			...
			struct fwnode_handle *child, enum gpiod_flags flags)
{
	...
	hAvioCtx->mipirst = devm_fwnode_gpiod_get_index(dev,
							child, "mipirst", 0, flags, "mipi");
	if (IS_ERR(hAvioCtx->mipirst)) {
		ret = PTR_ERR(hAvioCtx->mipirst);
		if (ret != -EPROBE_DEFER) {
			pr_err("MIPI Rst GPIO not found \n");
			hAvioCtx->mipirst = NULL;
			ret = 0;
		}
	}
...
EXPORT_SYMBOL(avio_module_mipirst_get_gpio_handle);
...
EXPORT_SYMBOL(avio_module_mipirst_set_gpio_val);
...
static int avio_drv_init(struct avio_device_t *avio_device)
{
	...
	avio_device->dev_class = SYNA_CLASS_CREATE(avio_device->dev_name);
...
static int avio_probe(struct platform_device *pdev)
{
	...
	ret = avio_sub_module_config(pdev);
	if (ret < 0)
		return ret;
...
static RET_TYPE avio_remove(struct platform_device *pdev)
{
	avio_trace("%s\n", __func__);
...
#ifndef CONFIG_SUPPORT_VPP_TA
	/* Disable for non TZ architecture */
	hAvioCtx->isTeeEnabled = 0;
#else
	/* Enable if TZ is enabled/supported */
...
static int __init avio_init(void)
{
	return platform_driver_register(&avio_driver);
}
late_initcall(avio_init)
