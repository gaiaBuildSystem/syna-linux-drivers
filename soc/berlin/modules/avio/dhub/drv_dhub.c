...
static int drv_dhub_suspend(void *h_dhub_ctx)
{
	avio_trace("%s:%d:\n", __func__, __LINE__);
	wrap_DhubEnableAutoPush(false, true, 60);
	dhub2nd_suspend(h_dhub_ctx, 1);

	return 0;
}

static int drv_dhub_resume(void *h_dhub_ctx)
{
	avio_trace("%s:%d:\n", __func__, __LINE__);
	dhub2nd_suspend(h_dhub_ctx, 0);
...
static const AVIO_MODULE_FUNC_TABLE dhub_drv_fops = {
	...
	.restore_state = drv_dhub_resume,
	.save_state = drv_dhub_suspend
};
