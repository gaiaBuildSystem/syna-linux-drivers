				...
				int order)
{
	...
	ctrl4.uCTRL4_bitmap32_en_R0 = 1;
	ctrl4.uCTRL4_bitmap32_en_R1 = 1;
	...
	do {
		...
		syna_lcdc_write(dev, RA_LCDC_bitmap32_ctrl_R0 + 4 * (index / 6), sel.u32);
		syna_lcdc_write(dev, RA_LCDC_bitmap32_ctrl_R1 + 4 * (index / 6), sel.u32);
		index += 6;
	} while(index < 30);

	sel5.uSEL_BIT_POS30 = bitmap_table[index + 0];
	sel5.uSEL_BIT_POS31 = bitmap_table[index + 1];
	syna_lcdc_write(dev, RA_LCDC_bitmap32_ctrl_R0 + 4 * (index / 6), sel5.u32);
	syna_lcdc_write(dev, RA_LCDC_bitmap32_ctrl_R1 + 4 * (index / 6), sel5.u32);
}

void syna_lcdc_cfg_dlr_fifoflush(struct syna_lcdc_dev *dev)
{
	T32LCDC_DYNCTRL_NF0 dynctrl_nf0;
	dynctrl_nf0.u32 = syna_lcdc_read(dev, RA_LCDC_DYNCTRL_NF0);
	//flush fifo every frame boundary
	dynctrl_nf0.uDYNCTRL_NF0_fifo_flush = 1;
	syna_lcdc_write(dev, RA_LCDC_DYNCTRL_NF0, dynctrl_nf0.u32);
	dynctrl_nf0.uDYNCTRL_NF0_fifo_flush = 0;
	syna_lcdc_write(dev, RA_LCDC_DYNCTRL_NF0, dynctrl_nf0.u32);
}
...
void syna_lcdc_cfg_dlr_init(struct syna_lcdc_dev *dev, int num)
{
	...
	dev->lcdcID = num;
	if (num == SYNA_LCDC_1) {
		avio_error("Invalid LCDC id : SYNA_LCDC_1\n");
	} else {
		dev->dmaRID = avioDhubChMap_vpp128b_LCDC2_Y_R;
		dev->intrID = avioDhubSemMap_vpp128b_vpp_intr0;
...
{
	unsigned int addr;
	T32avioVppGbl_INTR_CTRL lcdc_intr;
