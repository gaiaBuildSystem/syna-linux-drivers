...
static HRESULT wait_vpp_primary_vsync(int Id) {
	...
	pSem = (!Id) ? &vpp_ctx.vsync_sem : &vpp_ctx.vsync1_sem;
...
static void avio_devices_vpp_process_msg(VPP_CTX *hVppCtx, CC_MSG_t *pMsg)
{
...
static int avio_devices_vpp_isr(UNSG32 intrMask, void *pArgs)
{
			  ...
			  unsigned long arg, long *retVal)
{
	...
	*retVal = 0;
	...
	case VPP_IOCTL_GET_MSG:
		...
		if (copy_to_user((void __user *) arg, &msg, sizeof(CC_MSG_t)))
			VPP_MODULE_RETURN_WITH_EFAULT();
		break;
	...
	case HDMITX_IOCTL_UPDATE_HPD:
		{
			...
			/*HPD handled only if ampservice is active*/
			up(&vpp_ctx.hpd_sem);
			break;
		}
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0)
static const struct proc_ops avio_driver_config_fops = {
	.proc_open = avio_config_proc_open,
	.proc_read = seq_read,
	.proc_lseek = seq_lseek,
	.proc_release = single_release,
	.proc_write = avio_config_proc_write,
};

static const struct proc_ops avio_driver_status_fops = {
	.proc_open = avio_status_proc_open,
	.proc_read = seq_read,
	.proc_lseek = seq_lseek,
	.proc_release = single_release,
};

static const struct proc_ops avio_driver_detail_fops = {
	.proc_open = avio_detail_proc_open,
	.proc_read = seq_read,
	.proc_lseek = seq_lseek,
	.proc_release = single_release,
};
