...
static struct reg_item avio_gbl[] = {
	/* use chipset specific configs */
	AVIO_DEFAULT_GBL_CONFIG
};
...
static int aio_apll_config(struct aio_priv *aio)
{
	...
	for (i = 0; i < ARRAY_SIZE(avio_gbl); i++)
		writel_relaxed(avio_gbl[i].v, aio->gbl_base + avio_gbl[i].addr);

	AIO_PLL_POWER_CTRL(true);
	return 0;
}
...
static int drv_aio_init_clk(struct aio_priv *aio)
{
	...
	for (i = 0; i < AIO_CLK_MAX; i++) {
		...
		if (aio->clk_rate[i])
			clk_set_rate(l_clk, aio->clk_rate[i]);
	}
...
static int drv_aio_exit_clk(struct aio_priv *aio)
{
	struct clk *l_clk;
	int i;
	...
	for (i = 0; i < AIO_CLK_MAX; i++) {
		l_clk = aio->a_clk[i];
		if (IS_ERR(l_clk)) {
			avio_trace("clk(%d) disable error\n", i);
			clk_disable_unprepare(l_clk);
			clk_put(l_clk);
		} else {
			avio_trace("clk(%d) disable\n", i);
			clk_disable_unprepare(l_clk);
			clk_put(l_clk);
		}
	}
...
static void drv_aio_exit(void *h_aio_ctx)
{
	...
	AIO_PLL_POWER_CTRL(false);
	drv_aio_exit_clk(aio);
...
{
	const char *clk_name[AIO_CLK_MAX] = { "audio0_clk", "audio1_clk",
					      "aio_sysclk" };
	unsigned long default_rate[AIO_CLK_MAX] = { DEFAULT_APLL0_RATE,
						    DEFAULT_APLL1_RATE, 0 };
	int i, j, ret;
	struct clk *l_clk;
	...
	for (i = 0; i < AIO_CLK_MAX; i++) {
		...
		if (of_property_read_u32_index(np, "clock-rates", i,
					       &aio->clk_rate[i])) {
			pr_err("aio clock-rates not found, use default rate\n");
			aio->clk_rate[i] = default_rate[i];
		}
...
int avio_module_drv_aio_probe(struct platform_device *pdev)
{
	...
	aio = devm_kzalloc(dev, sizeof(struct aio_priv), GFP_KERNEL);
	if (!aio) {
	...
	aio->context = devm_kzalloc(dev, sizeof(struct reg_item) * aio->item_cn,
				    GFP_KERNEL);
	...
	aio->dev = dev;
	avio_sub_module_register(AVIO_MODULE_TYPE_AIO, AIO_MODULE_NAME, aio,
				 &aio_drv_fops);
	return 0;
}
