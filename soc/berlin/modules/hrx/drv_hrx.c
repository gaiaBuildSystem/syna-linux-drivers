...
#define AVIO_HDMIRX_WRAP_REG_BASE MEMMAP_AVIO_REG_BASE + \
	AVIO_MEMMAP_AVIO_HDMIRX_WRAP_REG_BASE
#define AVIO_HDMIRX_REG_BASE MEMMAP_AVIO_REG_BASE + \
	AVIO_MEMMAP_AVIO_HDMIRX_REG_BASE

static unsigned int hrx_pwr_val = 1;
...
#if (LINUX_VERSION_CODE > KERNEL_VERSION(5, 10, 0))
			dma_buf_vunmap(hrx->shm.buf, &map);
#else
			dma_buf_kunmap(hrx->shm.buf, 0, hrx->shm.p);
#endif
			dma_buf_end_cpu_access(
			hrx->shm.buf, DMA_BIDIRECTIONAL);
			shm_put(hrx->client, gid);
...
static int drv_hrx_open(struct inode *inode, struct file *file)
{
	...
	if (!is_avio_driver_initialized()) {
		atomic_dec_return(&hrx_dev_refcnt);
		hrx_error("avio_driver not ready - exit\n");
		return -EINVAL;
	}
...
static void hrx_disable_irq(struct hrx_priv *hrx)
{
	/* disable all enabled interrupt */
	disable_irq_nosync(hrx->otg_intr);
	disable_irq_nosync(hrx->hdmirx_intr);
	disable_irq_nosync(hrx->ytg_intr);
	disable_irq_nosync(hrx->uvtg_intr);
	disable_irq_nosync(hrx->itg_intr);
	disable_irq_nosync(hrx->mic3_intr);
}

static void hrx_enable_irq(struct hrx_priv *hrx)
{
	enable_irq(hrx->otg_intr);
	enable_irq(hrx->hdmirx_intr);
	enable_irq(hrx->ytg_intr);
	enable_irq(hrx->uvtg_intr);
	enable_irq(hrx->itg_intr);
	enable_irq(hrx->mic3_intr);
}

static int hrx_suspend(struct device *dev)
{
	struct hrx_priv *hrx = get_hrx_priv(dev);
	hrx_trace("hrx_suspend\n");

	if (unlikely(!hrx)) {
		hrx_error("hrx is NULL\n");
		return -EINVAL;
	}
	hrx_disable_irq(hrx);
	return 0;
...
static int hrx_resume(struct device *dev)
{
	struct hrx_priv *hrx = get_hrx_priv(dev);
	HRESULT rc = S_OK;
	CC_MSG_t msg = { 0, 0, 0 };

	hrx_trace("hrx_resume\n");
	...
	msg.m_Param1 = HDMIRX_INTR_WAKEUP;
	rc = AMPMsgQ_Add(&hrx->hHdmirxMsgQ, &msg);
	if (rc != S_OK) {
		hrx_error("Hdmirx MsgQ full\n");
		return -1;
	}
	hrx_trace("Pushed Hdmirx interrupt = %d", HDMIRX_INTR_WAKEUP);
	up(&hrx->hdmirx_sem);
	return 0;
}
#endif

static SIMPLE_DEV_PM_OPS(hrx_pmops, hrx_suspend,
                         hrx_resume);

void hrx_set_sram_power(unsigned int val)
{
	unsigned int addr;

	addr = AVIO_HDMIRX_WRAP_REG_BASE + RA_HDMI_RX_WRAP_videostramer_SRAMPWR;
	GA_REG_WORD32_WRITE(addr, val);

	addr = AVIO_HDMIRX_WRAP_REG_BASE + RA_HDMI_RX_WRAP_audiofifo_SRAMPWR;
	GA_REG_WORD32_WRITE(addr, val);

	addr = AVIO_HDMIRX_WRAP_REG_BASE + RA_HDMI_RX_WRAP_packetfifo_SRAMPWR;
	GA_REG_WORD32_WRITE(addr, val);

	addr = AVIO_HDMIRX_WRAP_REG_BASE + RA_HDMI_RX_WRAP_repeatermem_SRAMPWR;
	GA_REG_WORD32_WRITE(addr, val);

	addr = AVIO_HDMIRX_WRAP_REG_BASE + RA_HDMI_RX_WRAP_PHYSRAM_SRAMPWR;
	GA_REG_WORD32_WRITE(addr, val);
}

static int hrx_pwr_config(const char *val,
				const struct kernel_param *param)
{
	...
	if (hrx_pwr_val == 0) {
		hrx_error("power-off hrx: \n");
		hrx_set_sram_power(7);

		reg_addr = AVIO_HDMIRX_REG_BASE + HDMI_PHY_CONFIG;
		GA_REG_WORD32_WRITE(reg_addr, 0x103);

		reg_addr = AVIO_HDMIRX_REG_BASE + HDMI_EARCTX_PHY_CONFIG;
		GA_REG_WORD32_WRITE(reg_addr, 0x0);

		reg_addr = MEMMAP_AVIO_REG_BASE + AVIO_MEMMAP_AVIO_GBL_BASE + \
			RA_avioGbl_HDMI_CLK_EN;
		GA_REG_WORD32_READ(reg_addr, &reg_val);
		reg_val &= ~(MSK32avioGbl_HDMI_CLK_EN_HdmiRx);
		GA_REG_WORD32_WRITE(reg_addr, reg_val);
	} else {
		hrx_error("power-on hrx: \n");
		reg_addr = MEMMAP_AVIO_REG_BASE + AVIO_MEMMAP_AVIO_GBL_BASE + \
			RA_avioGbl_HDMI_CLK_EN;
		GA_REG_WORD32_READ(reg_addr, &reg_val);
		reg_val |= MSK32avioGbl_HDMI_CLK_EN_HdmiRx;
		...
		hrx_set_sram_power(0);

		reg_addr = AVIO_HDMIRX_REG_BASE + HDMI_PHY_CONFIG;
		GA_REG_WORD32_WRITE(reg_addr, 0x8C00);

		reg_addr = AVIO_HDMIRX_REG_BASE + HDMI_EARCTX_PHY_CONFIG;
		GA_REG_WORD32_WRITE(reg_addr, 0x3);
...
static const struct kernel_param_ops hrx_sys_pwr_config= {
	.set = hrx_pwr_config,
	.get = &param_get_uint,
};


module_param_cb(hrx_pwr, &hrx_sys_pwr_config, &hrx_pwr_val, 0644);
