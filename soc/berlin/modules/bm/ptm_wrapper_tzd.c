...
#define PTM_TA_IMAGE "ta/libptm.ta"
...
static int ptm_load_ta(struct ptm_context *ptm_ctx, struct device *dev)
{
	TEEC_Context *tee_ctx = &ptm_ctx->teec_ctx;
	const struct firmware *fw = NULL;
	TEEC_SharedMemory fw_shm = { 0, };
	TEEC_Parameter tee_param = { 0, };
	int ret;

	ret = request_firmware(&fw, PTM_TA_IMAGE, dev);
	if (ret) {
	...
	if (TEEC_ERROR_ACCESS_CONFLICT == ret) {
		pr_warn("PTM TA has been loaded\n");
		ret = 0;
	} else if (ret) {
		pr_err("can't register TA: 0x%08x\n", ret);
	}
...
int ptm_wrapper_init(struct ptm_context *ptm_ctx, struct device *dev)
{
	...
	ret = ptm_load_ta(ptm_ctx, dev);
	if (ret) {
		pr_err("failed to load ptm ta %x\n", ret);
		goto error;
	}
