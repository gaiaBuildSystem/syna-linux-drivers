...
struct tzd_drv {
	...
	dev_t tzd_device_num;
	struct cdev tzd_cdev;
...
static int tzd_open(struct inode *inode, struct file *file)
{
	...
	struct tzd_drv  *tzd = container_of(inode->i_cdev,
				struct tzd_drv, tzd_cdev);
...
static int kernel_tzd_open(struct tzd_drv *tzd)
{
	struct tzd_dev_file *new_dev;
...
static int tzd_probe(struct platform_device *pdev)
{
	...
	tzd = devm_kzalloc(&pdev->dev, sizeof(struct tzd_drv), GFP_KERNEL);
	if (!tzd)
		return -ENOMEM;
	...
	cdev_init(&tzd->tzd_cdev, &tzd_fops);
	tzd->tzd_cdev.owner = THIS_MODULE;

	ret = cdev_add(&tzd->tzd_cdev,
	...
	tzd->class_dev = class_dev;
	tzd->driver_class = driver_class;
	tzd->tzd_device_num = tzd_device_num;
	tzd->pdev = pdev;

	platform_set_drvdata(pdev, tzd);

	ret = kernel_tzd_open(tzd);
	if (ret < 0) {
...
static int tzd_remove(struct platform_device *pdev)
{
	...
	kernel_tzd_release();
	cdev_del(&tzd->tzd_cdev);
	device_destroy(tzd->driver_class, tzd->tzd_device_num);
	class_destroy(tzd->driver_class);
	unregister_chrdev_region(tzd->tzd_device_num, 1);
...
MODULE_SOFTDEP("pre: cma_heap system_heap cma_cust_heap system_cust_heap berlin_heap");
