...
static int phy_dolphin_pcie_init(struct phy *phy)
{
	u32 val;
	int count;
	void __iomem *addr;
...
static int phy_dolphin_pcie_power_off(struct phy *phy)
{
	u32 val;
	void __iomem *addr;
	struct phy_berlin_pcie_priv *priv = phy_get_drvdata(phy);
...
static const struct phy_ops phy_dolphin_pcie_ops = {
	.init		= phy_dolphin_pcie_init,
	.power_off	= phy_dolphin_pcie_power_off,
	.owner		= THIS_MODULE,
};
