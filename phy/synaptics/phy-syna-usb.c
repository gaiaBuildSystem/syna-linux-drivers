...
#define DOLPHIN_USB_PHY_CTRL0		0x0
#define DOLPHIN_USB_PHY_CTRL1		0x4
#define DOLPHIN_USB_PHY_CTRL2		0x8
#define DOLPHIN_PHY_CTRL0_DEF		0x533DADF0
#define DOLPHIN_PHY_CTRL1_DEF		0x01B10000
#define DOLPHIN_PHY_CTRL2_SIDDQ		BIT(1)

static int phy_dolphin_usb_power_on(struct phy *phy)
{
	struct phy_syna_usb2_priv *priv = phy_get_drvdata(phy);

	reset_control_assert(priv->rst);
	/* setup USB_PHY_CTRL0 */
	writel(DOLPHIN_PHY_CTRL0_DEF, priv->base + DOLPHIN_USB_PHY_CTRL0);
	/* setup USB_PHY_CTRL1 */
	writel(DOLPHIN_PHY_CTRL1_DEF, priv->base + DOLPHIN_USB_PHY_CTRL1);
	reset_control_deassert(priv->rst);
	udelay(100);
...
static int phy_dolphin_usb_power_off(struct phy *phy)
{
	struct phy_syna_usb2_priv *priv = phy_get_drvdata(phy);
	u32 val;
	...
	val |= DOLPHIN_PHY_CTRL2_SIDDQ;
	writel(val, priv->base + DOLPHIN_USB_PHY_CTRL2);

	return 0;
}

static const struct phy_ops phy_dolphin_usb_ops = {
...
static int phy_dolphin_usb3_power_on(struct phy *phy)
{
	struct phy_syna_usb2_priv *priv = phy_get_drvdata(phy);
	u32 val;
...
static int phy_dolphin_usb3_power_off(struct phy *phy)
{
	struct phy_syna_usb2_priv *priv = phy_get_drvdata(phy);
	u32 val;
...
static int phy_dolphin_usb3_init(struct phy *phy)
{
	struct phy_syna_usb2_priv *priv = phy_get_drvdata(phy);
...
static const struct phy_ops phy_dolphin_usb3_ops = {
	.init		= phy_dolphin_usb3_init,
	.power_on	= phy_dolphin_usb3_power_on,
	.power_off	= phy_dolphin_usb3_power_off,
	.owner		= THIS_MODULE,
};
				   ...
				   enum phy_mode mode, int submode)
{
	...
	case PHY_MODE_USB_HOST:
		val = readl(priv->base + SL261X_USB_PHY_CTRL1);
		val |= (PHY_USB_CTRL1_DPPULLDOWN | PHY_USB_CTRL1_DMPULLDOWN);
		val &= ~PHY_USB_CTRL1_IDDIG;
		writel(val, priv->base + SL261X_USB_PHY_CTRL1);
		break;
...
static const struct of_device_id phy_syna_usb2_of_match[] = {
	...
	},
	{
		...
		.data = &phy_dolphin_usb_ops,
	},
	{
		.compatible = "syna,dolphin-usb3-phy",
		.data = &phy_dolphin_usb3_ops,
	},
	{
		.compatible = "syna,sl261x-usb2-phy",
		.data = &phy_sl261x_usb_ops,
	},
	{
		/* for FPGA test */
...
static int phy_syna_usb2_probe(struct platform_device *pdev)
{
	...
	if (of_device_is_compatible(dev->of_node, "syna,berlin-dummy-usb2-phy"))
