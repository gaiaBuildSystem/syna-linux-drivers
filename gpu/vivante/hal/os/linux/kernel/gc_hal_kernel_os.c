...
{
    idr_preload(GFP_KERNEL | __GFP_NOWARN);

    spin_lock(&Database->lock);

    *Id = idr_alloc(&Database->idr, KernelPointer, 1, 0,
...
)
{
    ...
    if (do_munmap(mm, (unsigned long)Logical, Size, NULL) < 0)
    {
        gcmkTRACE_ZONE(
    ...
    )
{
    ...
    if (mdlMap->vmaAddr == gcvNULL)
    {
        ...
        if (IS_ERR(mdlMap->vmaAddr))
        {
            ...
            mm_populate((unsigned long)mdlMap->vmaAddr, populate);
        }
    ...
    )
{
    ...
    if (mdlMap->vmaAddr == gcvNULL)
    {
        ...
        mdlMap->vma->vm_flags |= VM_DONTEXPAND | VM_DONTDUMP;
#if !gcdPAGED_MEMORY_CACHEABLE
        ...
        else
        {
            ...
            if (result <=0 || result < pageCount)
            {
                ...
                if (vma && (vma->vm_flags & VM_PFNMAP) )
                {
                    ...
                    pud_t * pud = pud_offset(p4d_offset(pgd, memory), memory);
                    if (pud)
                    {
    ...
    )
{
    ...
    else
    {
        ...
        prepare_to_swait_exclusive(&signal->obj.wait, &wait, TASK_INTERRUPTIBLE);
