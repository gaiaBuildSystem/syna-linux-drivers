...
extern int mali_counter_target;
module_param(mali_counter_target, int, S_IRUSR | S_IWUSR | S_IWGRP | S_IRGRP | S_IROTH); /* rw-rw-r-- */
MODULE_PARM_DESC(mali_counter_target, "Target for the performance counter");
...
static int
dspg_mali_remove(struct platform_device *pdev)
{
...
static struct work_struct freq_change_work;

static struct mali_gpu_clk_item mali_dvfs[] = {
	{ 60, 1025000},
	{120, 1025000},
	{180, 1025000},
	{240, 1025000},
...
static void
mali_report_gpu_clock_info(struct mali_gpu_clock **data)
{
	mali_clock_items.num_of_steps = ARRAY_SIZE(mali_dvfs);
	mali_clock_items.item = mali_dvfs;
	*data = &mali_clock_items;
}

static int
mali_gpu_get_clock_step(void)
{
	return cur_freq_index;
}

static void
mali_freq_change_work(struct work_struct *work)
{
	clk_set_rate(gpu_clk, mali_dvfs[cur_freq_index].clock * 1000000);
}

static int
mali_gpu_set_clock_step(int freq_index)
{
	cur_freq_index = freq_index;
	schedule_work(&freq_change_work);
...
static int
dspg_mali_probe(struct platform_device *pdev)
{
	...
	gpu_clk = devm_clk_get(&pdev->dev, NULL);
	if (IS_ERR(gpu_clk)) {
...
#ifdef CONFIG_MALI_DVFS
	mali_gpu_data.control_interval = 100; /* ms */
	mali_gpu_data.get_clock_info = mali_report_gpu_clock_info;
	mali_gpu_data.get_freq = mali_gpu_get_clock_step;
	mali_gpu_data.set_freq = mali_gpu_set_clock_step;
#endif

	/* maximum limit of memory the Mali GPU driver can allocate from the
	 * OS */
	if (of_property_read_u32(pnode, "mali-mem-size", &size))
		mali_gpu_data.shared_mem_size = 0x0E000000;
	else
		mali_gpu_data.shared_mem_size = size;

	if (of_property_read_u32(pnode, "mali-fb-start", &size))
		mali_gpu_data.fb_start = 0x40000000;
	else
		mali_gpu_data.fb_start = size;

	if (of_property_read_u32(pnode, "mali-fb-size", &size))
		mali_gpu_data.fb_size = 0xC0000000;
	else
		mali_gpu_data.fb_size = size;

	if (platform_device_add_data(pdev, &mali_gpu_data,
				     sizeof(mali_gpu_data))) {
		MALI_PRINT_ERROR(("DSPG Mali memory registration failed.\n"));
		return -EEXIST;
	}

	mali_gpu_data.rc = devm_reset_control_get(&pdev->dev, NULL);
	if (IS_ERR(mali_gpu_data.rc))
		return PTR_ERR(mali_gpu_data.rc);
	ret = reset_control_deassert(mali_gpu_data.rc);
	if (ret)
		return ret;
	...
	if (of_property_read_u32(pnode, "gpu-clk-rate", &rate) == 0) {
		clk_set_rate(gpu_clk, rate);

		if (ARRAY_SIZE(mali_dvfs) < 2 ||
		    rate / 1000000 >
		    mali_dvfs[ARRAY_SIZE(mali_dvfs) - 2].clock)
			mali_dvfs[ARRAY_SIZE(mali_dvfs) - 1].clock =
								rate / 1000000;
	}
	INIT_WORK(&freq_change_work, mali_freq_change_work);
