...
void
mali_l2_cache_print_perf(struct timer_list *t)
{
	struct mali_group *group = from_timer(group, t, debug_timer_l2);
	struct mali_l2_cache_core *core0 = group->l2_cache_core[0];
	struct mali_l2_cache_core *core1 = group->l2_cache_core[1];

	if (!IS_ERR_OR_NULL(core0))
		MALI_DEBUG_PRINT(2,
			("Mali L2[0]: Register MALI400_L2_CACHE_REGISTER_PERFCNT_VAL0 = 0x%08X\n",
			 mali_hw_core_register_read(&core0->hw_core,
			 MALI400_L2_CACHE_REGISTER_PERFCNT_VAL0)));
	if (!IS_ERR_OR_NULL(core1))
		MALI_DEBUG_PRINT(2,
			("Mali L2[1]: Register MALI400_L2_CACHE_REGISTER_PERFCNT_VAL0 = 0x%08X\n",
			 mali_hw_core_register_read(&core1->hw_core,
			 MALI400_L2_CACHE_REGISTER_PERFCNT_VAL0)));
