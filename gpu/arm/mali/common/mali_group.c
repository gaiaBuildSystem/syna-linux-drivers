				     ...
				     u32 domain_index)
{
	...
	if (NULL != group) {
		...
		group->debug_timer = _mali_osk_timer_init();
		group->debug_timer_l2 = _mali_osk_timer_init();
		if (NULL != group->debug_timer) {
			_mali_osk_timer_setcallback(group->debug_timer,
						    mali_pp_print_registers,
						    (void *)group);
		} else {
			MALI_PRINT_ERROR(("could not initialize PP debug timer!\n"));
		}
		if (NULL != group->debug_timer_l2) {
			_mali_osk_timer_setcallback(group->debug_timer_l2,
						    mali_l2_cache_print_perf,
						    (void *)group);
		} else {
			MALI_PRINT_ERROR(("could not initialize L2 debug timer!\n"));
		}
#endif
...
#ifdef MALI_DEBUG_TIMERS
	if (NULL != group->debug_timer) {
		_mali_osk_timer_del(group->debug_timer);
		_mali_osk_timer_term(group->debug_timer);
	}

	if (NULL != group->debug_timer_l2) {
		_mali_osk_timer_del(group->debug_timer_l2);
		_mali_osk_timer_term(group->debug_timer_l2);
	}
#endif
...
static void mali_group_dump_core_status(struct mali_group *group)
{
	...
	if (group->l2_cache_core[0]) {
		MALI_PRINT(("Dump Group L2[0]\n"));
		for (i = 0; i < 0x30; i += 0x10) {
			MALI_PRINT(("0x%04x: 0x%08x 0x%08x 0x%08x 0x%08x\n", i,
				mali_hw_core_register_read(
					&group->l2_cache_core[0]->hw_core, i),
				mali_hw_core_register_read(
					&group->mmu->hw_core, i + 4),
				mali_hw_core_register_read(
					&group->mmu->hw_core, i + 8),
				mali_hw_core_register_read(
					&group->mmu->hw_core, i + 12)));
		}
	}

	if (group->l2_cache_core[1]) {
		MALI_PRINT(("Dump Group L2[1]\n"));
		for (i = 0; i < 0x30; i += 0x10) {
			MALI_PRINT(("0x%04x: 0x%08x 0x%08x 0x%08x 0x%08x\n", i,
				mali_hw_core_register_read(
					&group->l2_cache_core[0]->hw_core, i),
...
#ifdef MALI_DEBUG_TIMERS
	_mali_osk_timer_mod(group->debug_timer,
			    _mali_osk_time_mstoticks(1000));
	_mali_osk_timer_mod(group->debug_timer_l2,
			    _mali_osk_time_mstoticks(1000));
#endif
...
#ifdef MALI_DEBUG_TIMERS
	_mali_osk_timer_del_async(group->debug_timer);
	_mali_osk_timer_del_async(group->debug_timer_l2);
#endif
...
static void mali_group_timeout(struct timer_list *t)
{
	struct mali_group *group = NULL; // from_timer(group, t, timeout_timer);
	MALI_DEBUG_ASSERT_POINTER(group);
