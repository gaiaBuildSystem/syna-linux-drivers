/*
 * Copyright (C) 2011-2016, 2018 ARM Limited. All rights reserved.
 * 
...
void mali_pp_job_start(struct mali_pp_core *core, struct mali_pp_job *job, u32 sub_job, mali_bool restart_virtual)
{
	...
	if (mali_counter_target >= 0 && mali_counter_target < 64) {
		MALI_DEBUG_PRINT(3, ("Mali PP: Counter target %u\n",
				 mali_counter_target));
		mali_hw_core_register_write(&core->hw_core,
					MALI200_REG_ADDR_MGMT_PERF_CNT_0_SRC,
					(unsigned)mali_counter_target);
		mali_hw_core_register_write(&core->hw_core,
				MALI200_REG_ADDR_MGMT_PERF_CNT_0_ENABLE, 1);
	}
