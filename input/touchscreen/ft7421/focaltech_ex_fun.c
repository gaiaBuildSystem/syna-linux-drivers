...
static ssize_t
fts_debug_write(struct file *filp, const char __user *buff, size_t count,
		loff_t *ppos)
{
...
//printk("\n  zax v= %d \n",apk_debug_flag);
	...
	case PROC_UPGRADE:
	{
		...
		snprintf(upgrade_file_path, 128, "%s", writebuf + 1);
...
static ssize_t
fts_debug_read(struct file *filp, char __user *buff, size_t count,
	       loff_t *ppos)
{
	int ret = 0;
...
{
	#if (LINUX_VERSION_CODE > KERNEL_VERSION(3, 10, 0))
	fts_proc_entry = proc_create(PROC_NAME, 0, NULL, &fts_proc_fops);
	#else
	fts_proc_entry = create_proc_entry(PROC_NAME, 0, NULL);
	#endif
