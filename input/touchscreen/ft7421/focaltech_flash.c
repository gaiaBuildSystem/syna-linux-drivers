...
#if 0
static void
delay_qt_ms(unsigned long w_ms)
{
	unsigned long i;
	unsigned long j;
...
int
fts_6x36_ctpm_fw_upgrade(struct i2c_client *client, u8 *pbt_buf, u32 dw_lenth)
{
	u8 reg_val[2] = { 0 };
	u32 i = 0;
	u32 packet_number;
...
int  fts_5x46_ctpm_fw_upgrade(struct i2c_client * client, u8* pbt_buf, u32 dw_lenth)
{
	...
	for (k = 0; k < 1 /*FTS_UPGRADE_LOOP*/; k++) {
		...
		if (reg_val[0] != bt_ecc) {
			...
			printk(KERN_WARNING "k=%d,checksum fw_ecc=%X flash_ecc=%X i2c_ecc \n", k, reg_val[0], bt_ecc); //,reg_val_i2c[0]);
			break;
		}
	}
 ...
 ***********************************************************************/
static int
fts_GetFirmwareSize(char *firmware_name)
{
 ...
 */
static int
fts_ReadFirmware(char *firmware_name, unsigned char *firmware_buf)
{
	struct file *pfile = NULL;
	struct inode *inode;
	unsigned long magic;
 ...
 ***********************************************************************/
int
fts_ctpm_fw_upgrade_with_app_file(struct i2c_client *client,
				  char *firmware_name)
{
	u8 *pbt_buf = NULL;
	int i_ret = 0;
	...
	if (fts_updateinfo_curr.CHIP_ID == 0x55 ||
		...
		i_ret = fts_8606_ctpm_fw_write_pram(client, aucFW_PRAM_BOOT,
		...
		i_ret = fts_8716_ctpm_fw_write_pram(client, aucFW_PRAM_BOOT,
						    sizeof(aucFW_PRAM_BOOT));
 ...
 * Brief:  update project setting, only update these settings for COB project,
 *         or for some special case
 * Input: i2c info
 * Output: no
 * Return: fail <0
 ***********************************************************************/
