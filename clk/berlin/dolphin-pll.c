...
#define hw_to_dolphin_pll(hw) container_of(hw, struct dolphin_pll, hw)
#define hw1_to_dolphin_pll(hw) container_of(hw, struct dolphin_pll, hw1)

static inline u32 rdl(struct dolphin_pll *pll, u32 offset)
{
	return readl_relaxed(pll->ctrl + offset);
}

static inline void wrl(struct dolphin_pll *pll, u32 offset, u32 data)
{
	writel_relaxed(data, pll->ctrl + offset);
}

static unsigned long
dolphin_pll_clko_recalc_rate(struct clk_hw *hw,
			   unsigned long parent_rate)
{
	u32 val, dp, dm, dn;
	u64 frac;
	unsigned long rate;
	struct dolphin_pll *pll = hw_to_dolphin_pll(hw);
...
static unsigned long
dolphin_pll_clko1_recalc_rate(struct clk_hw *hw,
			    unsigned long parent_rate)
{
	u32 val, dp, dm, dn;
	u64 frac;
	unsigned long rate;
	struct dolphin_pll *pll = hw1_to_dolphin_pll(hw);
...
static long
dolphin_pll_clko_round_rate(struct clk_hw *hw, unsigned long rate,
			  unsigned long *parent)
{
	...
	if ((rate > FREQ_OUT_HIGH_LIMIT) || (rate < FREQ_OUT_LOW_LIMIT))
		return dolphin_pll_clko_recalc_rate(hw, *parent);
...
static long
dolphin_pll_clko1_round_rate(struct clk_hw *hw, unsigned long rate,
			  unsigned long *parent)
{
	u32 dp, dm, dn, gcdv;
	u64 frac, parent_rate = *parent;
	...
	if ((rate > FREQ_OUTF_HIGH_LIMIT) || (rate < FREQ_OUTF_LOW_LIMIT))
		return dolphin_pll_clko1_recalc_rate(hw, *parent);
...
static u32 dolphin_pll_get_pfdrange(u32 freq)
{
	u32 r;
...
static void dolphin_pll_update_setting(struct dolphin_pll *pll,
			unsigned long parent_rate,
			u32 dm, u32 dn, u32 frac, int dp0, int dp1)
{
	...
	range = dolphin_pll_get_pfdrange(freq);
...
static int
dolphin_pll_set_rates(struct dolphin_pll *pll, unsigned long rate0,
			...
			unsigned long parent_rate)
{
	...
	dolphin_pll_update_setting(pll, parent_rate,
				dm - 1, dn - 1,
				frac, dp0 - 1, dp1 - 1);
	return 0;
}

static void dolphin_pll_bypass_and_pd(struct dolphin_pll *pll)
{
	u32 val;
...
static int dolphin_pll_try_bypass(struct dolphin_pll *pll, unsigned long rate,
					    int cur, int pair)
{
...
static bool dolphin_pll_use_preset_param(struct dolphin_pll *pll,
					unsigned long parent_rate,
...
static int
dolphin_pll_clko1_set_rate(struct clk_hw *hw, unsigned long rate,
			unsigned long parent_rate)
{
	struct dolphin_pll *pll = hw1_to_dolphin_pll(hw);
	u32 pll_set_flag;
	...
	if (dolphin_pll_try_bypass(pll, rate, 1, 0) == PLL_BYPASS_PD)
		return 0;

	if (dolphin_pll_use_preset_param(pll, parent_rate, rate, 1))
		return 0;
	...
	ret = dolphin_pll_set_rates(pll, rate0, rate1, pll_set_flag, parent_rate);
	return ret;
}

static int
dolphin_pll_clko_set_rate(struct clk_hw *hw, unsigned long rate,
			unsigned long parent_rate)
{
	struct dolphin_pll *pll = hw_to_dolphin_pll(hw);
	u32 pll_set_flag;
	...
	if (dolphin_pll_try_bypass(pll, rate, 0, 1) == PLL_BYPASS_PD)
		return 0;

	if (dolphin_pll_use_preset_param(pll, parent_rate, rate, 0))
		return 0;
...
static int dolphin_pll_restore(struct dolphin_pll *pll)
{
	unsigned long flags;
	u32 val, timeout;
...
static int dolphin_pll_suspend(struct device *dev)
{
	struct dolphin_pll *pll = dev_get_drvdata(dev);
...
static int dolphin_pll_resume(struct device *dev)
{
	struct dolphin_pll *pll = dev_get_drvdata(dev);
	...
	if (pll->bypassed) {
		...
		return dolphin_pll_restore(pll);
	}
	return 0;
}

static const struct dev_pm_ops dolphin_plls_pm_ops = {
	SET_SYSTEM_SLEEP_PM_OPS(dolphin_pll_suspend, dolphin_pll_resume)
};

#define DEV_PM_OPS	(&dolphin_plls_pm_ops)
#else
#define DEV_PM_OPS	NULL
#endif /* CONFIG_PM_SLEEP */

static const struct of_device_id dolphin_pll_match_table[] = {
	{ .compatible = "syna,dolphin-pll"},
	{ }
};
MODULE_DEVICE_TABLE(of, dolphin_pll_match_table);

static struct platform_driver dolphin_pll_driver = {
	.probe		= dolphin_pll_setup,
	.driver		= {
		.name	= "syna-dolphin-pll",
		.of_match_table = dolphin_pll_match_table,
		.pm	= DEV_PM_OPS,
	},
};
