...
void __init of_dspg_clk_gated_div_setup(struct device_node *node)
{
	...
	u8 div_flags = 0, mux_flags = 0, div_shift = 0, div_width;
	unsigned long flags = 0;
	const struct clk_ops *ops, *mux_ops;
	u32 tmp;
	...
	of_dspg_get_mux_flags(node, &mux_flags);
	...
	if (num_parents > 1) {
		clk_mux = dspg_create_clk_mux(node, 2, &_lock);
		if (IS_ERR_OR_NULL(clk_mux)) {
			kfree(div_table);
			kfree(clk_div);
			kfree(clk_gate);
			kfree(parent_names);
		...
		if (mux_flags & CLK_MUX_READ_ONLY)
