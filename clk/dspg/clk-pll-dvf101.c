...
static int dvf101_pll_determine_rate(struct clk_hw *hw,
				     struct clk_rate_request *req)
{
	...
	rate = _dvf101_pll_round_rate(req->best_parent_rate, req->rate,
				      NULL, NULL, NULL, NULL);
	if (rate < 0)
		return rate;
	req->rate = rate;
...
static const struct clk_ops dspg_dvf101_pll_ops = {
	...
	.determine_rate = dvf101_pll_determine_rate,
#if (LINUX_VERSION_CODE < KERNEL_VERSION(7,1,0))
