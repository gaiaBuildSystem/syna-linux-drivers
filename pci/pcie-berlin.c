...
static int berlin_pcie_host_init(struct dw_pcie_rp *pp)
{
	u32 val;
	int ret;
...
static int dolphin_pcie_host_init(struct dw_pcie_rp *pp)
{
	u32 val;
	int ret;
	struct dw_pcie *pci = to_dw_pcie_from_pp(pp);
				...
				struct platform_device *pdev)
{
	...
	struct dw_pcie_rp *pp = &priv->pci.pp;
...
#else
	berlin_pcie_host_ops.init = priv->data->host_init;
#endif
...
static int berlin_pcie_start_link(struct dw_pcie *pci)
{
	struct berlin_pcie *priv = to_berlin_pcie(pci);
	u32 val;
...
static const struct dw_pcie_ops dw_pcie_ops = {
	...
	.start_link = berlin_pcie_start_link,
};
...
static const struct berlin_pcie_data dolphin_pcie = {
	.intr_status = 0xC,
	.intr_status1 = 0x14,
	.intr_mask = 0x10,
	.intr_mask1 = 0x18,
...
static void berlin_pcie_shutdown(struct platform_device *pdev)
{
	berlin_pcie_suspend_noirq(&pdev->dev);
}
...
static RET berlin_pcie_remove(struct platform_device *pdev)
{
